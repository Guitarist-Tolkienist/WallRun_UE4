...
#include "GameFramework/CharacterMovementComponent.h" // for FXRMotionControllerBase::RightHandSourceId
...
void AWallRunCharacter::GetWallRunSideAndDirection(const FVector& HitNormal, EWallRunSide& OutSide, FVector& OutDirection) const
{
	if (FVector::DotProduct(HitNormal, GetActorRightVector()) > 0) {
		...
		GEngine->AddOnScreenDebugMessage(-1, 1.0f, FColor::Red, TEXT("Capsule Hit! Side LEft"));
	}
	else {
		OutSide = EWallRunSide::Right;
		OutDirection = FVector::CrossProduct(FVector::UpVector.GetSafeNormal(), HitNormal);
		GEngine->AddOnScreenDebugMessage(-1, 1.0f, FColor::Red, TEXT("Capsule Hit! Side Right"));
	}
}
...
bool AWallRunCharacter::AreRequiredKeyDown(EWallRunSide side) const
{
	if (ForwardAxis < 0.1f) {
		return false;
	}
...
void AWallRunCharacter::StartWallRun(EWallRunSide Side, const FVector& Direction) {
	GEngine->AddOnScreenDebugMessage(-1, 1.0f, FColor::Purple, TEXT("Start WallRun"));
	BeginCameraTilt();
...
void AWallRunCharacter::UpdateWallRun() 
{
	...
	FVector EndPostition = StartPosition + LineTraceLength * LineTraceDirection;

	FCollisionQueryParams QueryParams;
	QueryParams.AddIgnoredActor(this);

	if (GetWorld()->LineTraceSingleByChannel(HitResult, StartPosition, EndPostition, ECC_Visibility, QueryParams)) 
	{
		EWallRunSide Side = EWallRunSide::None;
		FVector Direction = FVector::ZeroVector;
...
void AWallRunCharacter::UpdateCameraTilt(float Value)
{
	...
	GetController()->ClientSetRotation(CurrentControlRotation);
}
